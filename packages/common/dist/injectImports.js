...
function injectImports(source, isTypeOnly = false) {
  ...
  return paths.map(([path, value]) => {
        ...
        .map((sub) => {
          ...
          if (types_1.isObject(sub)) {
            ...
            return `${syntax_1.shouldBeVariableName(
              firstKey
            )} as ${syntax_1.shouldBeVariableName(sub[firstKey])}`;
          }
        })
    ...
    if (types_1.isObject(value)) {
      ...
      if (value["*"]) {
        return `${t} * as ${syntax_1.shouldBeVariableName(
          value["*"]
        )} from "${path}";`;
      }
      ...
      if (subs.length) {
        return `${t} { ${subs
          .map(
            ([name, alias]) =>
              `${syntax_1.shouldBeVariableName(
                name
              )} as ${syntax_1.shouldBeVariableName(alias)}`
          )
          .join(", ")} } from "${path}";`;
      }
    }
